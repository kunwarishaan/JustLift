...
export default function PlayerSetup({ initialNames, onContinue, onSkip, onShowStats }) {
  ...
  return (
    <section className="game-screen" aria-label="Player names">
      ...
      <form onSubmit={(event) => { event.preventDefault(); onContinue(names) }}>
        ...
        <div className="game-screen__actions">
          ...
          <button type="button" onClick={onSkip}>Skip names</button>
          <button type="button" onClick={onShowStats}>Stats</button>
        </div>
      </form>
    </section>
  )
